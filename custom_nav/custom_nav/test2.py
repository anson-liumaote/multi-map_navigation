...
class NavHandler(Node):
    def __init__(self):
        ...
        self.declare_parameter('map_1.map_url_param', '')

        self.declare_parameter('map_1.initial_pose.x', 0.0)
        self.declare_parameter('map_1.initial_pose.y', 0.0)
        self.declare_parameter('map_1.initial_pose.z', 0.0)
        self.declare_parameter('map_1.initial_pose.yaw', 1.0)
        self.declare_parameter('map_1.nav_pose.x', 0.0)
        self.declare_parameter('map_1.nav_pose.y', 0.0)
        self.declare_parameter('map_1.nav_pose.z', 0.0)
        self.declare_parameter('map_1.nav_pose.yaw', 1.0)
    ...
    def id2id_callback(self, request, response):
        ...
        if request.a == 1:
            self.nav_pose_x = self.get_parameter('map_1.nav_pose.x'.format(str(request.b))).get_parameter_value().double_value
            self.nav_pose_y = self.get_parameter('map_1.nav_pose.y'.format(str(request.b))).get_parameter_value().double_value
            self.nav_pose_z = self.get_parameter('map_1.nav_pose.z'.format(str(request.b))).get_parameter_value().double_value
            self.nav_pose_yaw = self.get_parameter('map_1.nav_pose.yaw'.format(str(request.b))).get_parameter_value().double_value
            self.setPose()  # for navigation pose
            self.runNavigation()
        elif request.a == 2:
            self.req.map_url = self.get_parameter('map_1.map_url_param'.format(str(request.b))).get_parameter_value().string_value
            self.initial_pose_x = self.get_parameter('map_1.initial_pose.x'.format(str(request.b))).get_parameter_value().double_value
            self.initial_pose_y = self.get_parameter('map_1.initial_pose.y'.format(str(request.b))).get_parameter_value().double_value
            self.initial_pose_z = self.get_parameter('map_1.initial_pose.z'.format(str(request.b))).get_parameter_value().double_value
            self.initial_pose_yaw = self.get_parameter('map_1.initial_pose.yaw'.format(str(request.b))).get_parameter_value().double_value
            # self.switchMap()
            self.send_request()
            self.pubInitialPose()
        response.sum = 1
